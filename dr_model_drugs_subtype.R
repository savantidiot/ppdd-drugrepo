...
for (obj in 1:length(subtype_objects)){
  ...
  model_data_subtype <- model_days_new[which( substr(colnames(brca_cancer),1,12) %in% subtype_objects[obj]$bcr_patient_barcode ),]
  ...
  for (x in 1:165){
    ...
    try_err <- try (
      ...
      else{model_cox <- get_index(predictor_matrix, 5)}
      , silent = T)
  ...
  # write to file
  cox_drug_subtype_file <- paste0("cox_drug_results_subtype",obj,".txt")
