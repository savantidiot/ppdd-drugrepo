...
model_data_subtype <- model_days_new[which( substr(colnames(brca_cancer),1,12) %in% her2_patients$bcr_patient_barcode ),]
...
for (l in gene_len){
  ...
  for (x in 1:3){
    ...
    try_err <- try (
      model_cox <- get_index(predictor_matrix, 5) # for Her2, because of small sample size, we use 5-fold CV
      , silent = T)
  ...
  # write to file
  cox_random_file_subtype <- paste0("cox_random_results_subtype_her2",l,".txt")
